...
def test_update_invalid_fields(client, key, value):
    update = client.patch(f"/api/analysis/{uuid.uuid4()}", json={key: value, "version": str(uuid.uuid4())})
...
def test_update_invalid_node_fields(client, key, value):
    update = client.patch(f"/api/analysis/{uuid.uuid4()}", json={"version": str(uuid.uuid4()), key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
    assert key in update.text


def test_update_invalid_uuid(client):
    update = client.patch("/api/analysis/1", json={"version": str(uuid.uuid4())})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_update_nonexistent_uuid(client):
    update = client.patch(f"/api/analysis/{uuid.uuid4()}", json={"version": str(uuid.uuid4())})
    assert update.status_code == status.HTTP_404_NOT_FOUND
