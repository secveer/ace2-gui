...
def test_update_invalid_fields(client, key, value):
    update = client.patch(f"/api/alert/tool/instance/{uuid.uuid4()}", json={key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_update_invalid_uuid(client):
    update = client.patch("/api/alert/tool/instance/1", json={"value": "test"})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_update_duplicate_unique_fields(client, key):
    ...
    client.post("/api/alert/tool/instance/", json=create1_json)

    create2_json = {"value": "test2"}
    create2 = client.post("/api/alert/tool/instance/", json=create2_json)
...
def test_update_nonexistent_uuid(client):
    update = client.patch(f"/api/alert/tool/instance/{uuid.uuid4()}", json={"value": "test"})
...
def test_update(client, key, initial_value, updated_value):
    ...
    create = client.post("/api/alert/tool/instance/", json=create_json)
