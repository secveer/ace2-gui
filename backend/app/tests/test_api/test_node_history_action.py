...
def test_create_node_history_action_with_uuid(client):
    ...
    get = client.get(f"/api/node/history_action/{u}")
    assert get.status_code == 200
...
def test_create_node_history_action_invalid_value(client):
    create = client.post("/api/node/history_action", json={"value": {"asdf": "asdf"}})
    assert create.status_code == 422


def test_create_node_history_action_missing_value(client):
    create = client.post("/api/node/history_action", json={})
    assert create.status_code == 422
...
def test_get_all_node_history_actions(client):
    ...
    client.post("/api/node/history_action", json={"value": "default"})
    client.post("/api/node/history_action", json={"value": "intel"})

    # Read them back
    get = client.get("/api/node/history_action")
    assert get.status_code == 200
...
def test_get_nonexistent_node_history_action(client):
    get = client.get(f"/api/node/history_action/{uuid.uuid4()}")
...
def test_update_nonexistent_node_history_action(client):
    update = client.put(f"/api/node/history_action/{uuid.uuid4()}", json={"value": "default"})
...
def test_delete_nonexistent_node_history_action(client):
    delete = client.delete(f"/api/node/history_action/{uuid.uuid4()}")
