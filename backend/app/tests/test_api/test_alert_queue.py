...
def test_create_alert_queue(client):
    ...
    create = client.post("/api/alert_queue", json={"value": "default"})
...
def test_create_alert_queue_duplicate_value(client):
    # Create an alert queue
    client.post("/api/alert_queue", json={"value": "default"})
...
def test_create_alert_queue_invalid_value(client):
    create = client.post("/api/alert_queue", json={"value": {"asdf": "asdf"}})
    assert create.status_code == 422


def test_create_alert_queue_missing_value(client):
    create = client.post("/api/alert_queue", json={})
    assert create.status_code == 422
...
def test_get_all_alert_queues(client):
    ...
    client.post("/api/alert_queue", json={"value": "default"})
    client.post("/api/alert_queue", json={"value": "intel"})
    
    # Read them back
    get = client.get("/api/alert_queue")
    assert get.status_code == 200
...
def test_update_alert_queue_duplicate_value(client):
    ...
    client.post("/api/alert_queue", json={"value": "default"})
    create = client.post("/api/alert_queue", json={"value": "intel"})
